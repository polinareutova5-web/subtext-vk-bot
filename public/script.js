const API_URL = "https://script.google.com/macros/s/AKfycbwEmqm-jCjnJB15Bgb6EfRQVGR29pGQC0YdHzBgaBy_T4Ny6YzLjOclSEanvYgTkdeHlg/exec";
...
async function loadSlots() {
  try {
    ...
    const mySlotContainer = document.getElementById('mySlot');

    // Очищаем контейнер
    slotsContainer.innerHTML = '';
    mySlotContainer.innerHTML = '';
    ...
    if (data.slots && data.slots.length > 0) {
      data.slots.forEach(slot => {
        ...
        if (data.mySlot) {
          // Если уже записан — кнопки неактивны
          btn.disabled = true;
          btn.style.opacity = '0.6';
        } else {
          btn.onclick = () => {
            if (confirm(`Записаться на ${slot.date} в ${slot.time}?`)) {
              bookSlot(slot.id);
            }
          };
        }
    ...
    // Отображаем текущий слот
    if (data.mySlot) {
      mySlotContainer.innerHTML = `
        <p style="color:#b71c1c; font-weight:bold;">
          ❤️ Ваш слот: <strong>${data.mySlot.date} · ${data.mySlot.time}</strong><br>
          <button class="buy-btn" onclick="cancelSlot('${data.mySlot.id}')">Отменить запись</button>
        </p>
      `;
    } else {
      mySlotContainer.innerHTML = '<p>Вы ещё не записаны на занятие</p>';
    }

  } catch (e) {
    console.error('Ошибка загрузки слотов:', e);
    document.getElementById('slots').innerHTML = '<p>❌ Не удалось загрузить слоты</p>';
  }
}
